...
async def main() -> None:
    ...
    teacher_result = await teacher_agent.run(task="""Explain what a probability 
    is to a beginner""")
    print("========Teacher:========", teacher_result.messages[-1].content)

    # Student responds to teacher
    student_result = await student_agent.run(task=f"""The teacher said: 
    ...
    teacher_result = await teacher_agent.run(task=f"""The student asked:
