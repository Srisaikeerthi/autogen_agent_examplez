...
async def main() -> None:
    ...
    tool_agent = AssistantAgent(
        ...
        system_message="You are a helpful assistant with access to various tools. Use them to help users with calculations, games, and interesting facts.",
        max_tool_iterations=3,  # Allow multiple tool calls
    )

    # FYI: max_tool_iterations is At most 10 iterations of tool calls before stopping the loop.
    ...
    tasks = [
        ...
        "Calculate the area of a circle with radius 3.5 and then roll 3 dice with 8 sides each"
    ]
